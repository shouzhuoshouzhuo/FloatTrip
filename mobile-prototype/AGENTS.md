...
## Qingzhou Product Decisions

- The full trip-detail screen is map-first: the route map fills the device screen and the itinerary lives in a persistent floating sheet.
- The itinerary sheet must support a compact peek state and an expanded readable state through both drag and tap controls; do not return this screen to a conventional cover-and-list page.
- Keep edit, add, date switching, route optimization, and continue-conversation actions available from the map-first detail experience.
