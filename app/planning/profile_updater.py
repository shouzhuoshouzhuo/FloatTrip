...
async def _async_profile_update(user_id: str, raw_query: str, model_name: str | None) -> None:
    def load_existing():
        with get_conn() as conn:
            return get_user_profile(user_id, conn)

    existing = await asyncio.to_thread(load_existing)

    llm = build_structured_llm(ProfileUpdateResult, model=model_name, temperature=0)
    result = await ainvoke_structured(llm, [
...
async def run_profile_update_agent(user_id: str, raw_query: str, model_name: str | None = None) -> None:
    ...
    try:
        await _async_profile_update(user_id, raw_query, model_name)
    except Exception:
