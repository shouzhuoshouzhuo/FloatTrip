...
- 从 Tavily 搜索结果抓取攻略网页，并提纯为去除页面噪声的 markdown 信息源
...
1. `collect_research()` 生成旅行搜索词，调用 Tavily 搜索攻略网页，并把网页正文清噪提纯为 markdown。
2. `extract_candidate_pool()` 把 markdown 喂给 DeepSeek 结构化抽取景点和偏好符合分，再由代码统计景点名在 markdown 中的提及频率并排序。
