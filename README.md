...
- 从攻略网页、搜索结果和本地 markdown 信息源抽取候选景点，而不是只返回一段聊天答案
- 按景点出现次数和用户偏好生成第一版候选景点排序信号
...
1. `collect_research()` 生成旅行搜索词，优先使用 SearchAPI Baidu，兜底 DuckDuckGo HTML 搜索，并抓取网页正文。
2. `extract_candidate_pool()` 从正文里抽取景点名，按出现次数和偏好得分排序。
3. `enrich_candidates_with_amap()` 调用高德 POI，把候选景点落到真实地点和坐标。
4. `build_daily_transit_route_plan()` 按天聚类、选择每日景点、查询高德路线、优化访问顺序。
5. `TripRoutePlanResponse` 返回可供前端地图和列表消费的结构化结果。
...
- `POST /api/planner/transit-routes`：输入已带 POI 坐标的候选景点，生成真实公交/步行路线
- `POST /api/planner/cluster-routes`：输入已带 POI 坐标的候选景点，只生成按天聚类和路线占位结果
...
- `route_mode`：`amap_transit` 或 `cluster_preview`
...
| 信息源 | SearchAPI Baidu、DuckDuckGo HTML、本地攻略 markdown、实验搜索脚本 |
...
agents     -> 后续大模型调研和自然语言行程生成边界
```

## 📁 Project Structure

- `backend/app/main.py`：FastAPI 应用入口，注册 CORS、健康检查和规划接口
- `backend/app/api/`：后端接口路由，目前包含规划相关 API
- `backend/app/schemas/`：请求、POI、地图点位、路线计划和行程文案的数据结构
- `backend/app/workflows/`：规划流程编排入口，串联完整生成、聚类预览和真实路线规划
- `backend/app/services/`：网页调研、候选池抽取和高德 POI 富化
- `backend/app/planning/`：确定性规划模块，包含分天聚类、路线矩阵、真实路线和访问顺序优化
...
- `backend/tests/`：后端 API、聚类、高德 provider 和路线规划测试
